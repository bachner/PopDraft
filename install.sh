...
if [[ "$SCRIPT_DIR" == *".app/Contents/Resources"* ]]; then
    ...
    if swiftc -O -o "$CONFIG_DIR/PopDraft" "$SCRIPT_DIR/scripts/PopDraft.swift" "$SCRIPT_DIR/scripts/Core.swift" -framework Cocoa -framework Carbon -framework WebKit -framework AVFoundation -framework Network 2>/dev/null; then
